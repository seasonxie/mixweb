...
var ViewMovie = Vue.extend({
    ...
    data: function () {
        this.$http.get("/api/" + this.$route.params.movieId + ".json").then(function (response) {
    ...
    methods: {
        remove: function (movie) {
            if (confirm("确定要删除该影片吗")) {
                this.$http.delete("/api/" + movie.id).then(function (response) {
...
var NewMovie = Vue.extend({
    ...
    methods: {
        save: function () {
            this.$http.post("/api/new", this.movie).then(function (response) {
                if (response.data) {
...
var EditMovie = Vue.extend({
    ...
    methods: {
        save: function () {
            this.$http.post("/api/edit", this.movie).then(function (response) {
                if (response.data == "success") {
